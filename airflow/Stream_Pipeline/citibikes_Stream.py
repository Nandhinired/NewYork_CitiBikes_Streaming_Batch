...
EXECUTION_YEAR = '{{ logical_date.replace(hour=logical_date.hour-1).strftime("%Y") }}'
EXECUTION_MONTH = '{{ logical_date.replace(hour=logical_date.hour-1).strftime("%-m") }}'
EXECUTION_DAY = '{{ logical_date.replace(hour=logical_date.hour-1).strftime("%-d") }}'
EXECUTION_HOUR = '{{ logical_date.replace(hour=logical_date.hour-1).strftime("%-H") }}'
EXECUTION_DATETIME_STR = '{{ logical_date.replace(hour=logical_date.hour-1).strftime("%m%d%H") }}'
...
STREAM_PROCESS = ['stationInfo', 'stationStatus', 'systemInfo']
...
with DAG(
    dag_id = f'citibikes_stream_processing',
    ...
    start_date=datetime(2023, 6, 9, 10),  # Start on May 1st at 10 am
    catchup=False,
    max_active_runs=1,
    ...
    for stream in STREAM_PROCESS:
        
        external_table_name = f'{stream}_external_table'
        gcp_gcs_bucket = f'citibike_real_time_en/{stream}'
        object_path = f'year={EXECUTION_YEAR}/month={EXECUTION_MONTH}/day={EXECUTION_DAY}/hour={EXECUTION_HOUR}'

        create_external_table_task = create_external_table(stream,
                                                        GCP_PROJECT_ID, 
                                                        BIGQUERY_DATASET, 
                                                        external_table_name, 
                                                        gcp_gcs_bucket, 
        ...
        delete_external_table_task = delete_external_table(stream,
                                                           GCP_PROJECT_ID, 
                                                           BIGQUERY_DATASET, 
                                                           external_table_name)
